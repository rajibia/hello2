$(document).ready(function () {
    ...
    function getPatientStatementTableData() {
        const dateRange = $('.date-range-display').text().trim() || 'All Dates';
        const table = document.querySelector('.table-responsive table');
        if (!table) {
            alert('No patient statement records found.');
            return null;
        }
        ...
        if (patientName) {
            ...
            patientDetails.forEach(detail => { patientInfo += `<p>${detail.textContent}</p>`; });
        }

        return { headers, rows, dateRange, patientInfo };
    }

    $('#exportCsv').click(function () {
        const data = getPatientStatementTableData();
        if (!data) return;
        ...
        link.download = `Patient_Statement_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    });
    ...
    $('#exportExcel').click(function () {
        ...
        let tableHTML = `
            <table border="1">
                <tr><th colspan="${data.headers.length}" style="font-size:16px">${APP_NAME} - Patient Statement Report</th></tr>
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `Patient_Statement_${new Date().toISOString().split('T')[0]}.xls`;
        link.click();
    });
    ...
    $('#exportPdf').click(function () {
        ...
        doc.setFontSize(14);
        doc.text('Patient Statement Report', pageWidth / 2, 60, { align: 'center' });
        doc.setFontSize(10);
        ...
        doc.save(`Patient_Statement_${new Date().toISOString().split('T')[0]}.pdf`);
    });
    ...
    $('#printReport').click(function () {
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <h3 style="text-align:center;">Patient Statement Report</h3>
                <p style="text-align:center;">Period: ${data.dateRange}</p>
                <div class="patient-info">${data.patientInfo}</div>
                ${tableHTML}
                <p style="text-align:center; margin-top:20px;">© ${new Date().getFullYear()} ${APP_NAME} | Hospital Management System</p>
            </body>
            </html>
        `);
