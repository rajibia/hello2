$(document).ready(function () {
    ...
    $('#exportCsv').click(function () {
        const data = getOpdStatementTableData();
        if (!data) return;
        ...
        link.download = `OPD_Statement_Report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    });
    ...
    $('#exportExcel').click(function () {
        ...
        let table = `
            <table border="1">
                <tr><th colspan="${data.headers.length}" style="font-size:16px">${APP_NAME} - OPD Statement Report</th></tr>
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `OPD_Statement_Report_${new Date().toISOString().split('T')[0]}.xls`;
        link.click();
    });
    ...
    $('#exportPdf').click(function () {
        ...
        doc.setFontSize(14);
        doc.text('OPD Statement Report', pageWidth / 2, 65, { align: 'center' });
        ...
        doc.save(`OPD_Statement_Report_${new Date().toISOString().split('T')[0]}.pdf`);
    });
