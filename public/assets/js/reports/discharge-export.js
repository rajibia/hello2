$(document).ready(function () {
    ...
    function getDischargeTableData() {
        ...
        const table = document.querySelector(`#${tabId} table`) || document.querySelector('div.tab-pane.active table');
        if (!table) {
            alert(`No ${reportType} discharge records found.`);
            return null;
        }
        ...
        return { headers, rows, reportType, dateRange };
    }

    
    $('#exportCsv').click(function () {
        const data = getDischargeTableData();
        if (!data) return;
        ...
        link.download = `${data.reportType}_Discharge_Report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    });
    ...
    $('#exportExcel').click(function () {
        ...
        let table = `
            <table border="1">
                <tr><th colspan="${data.headers.length}" style="font-size:16px">${APP_NAME} - ${data.reportType} Discharge Report</th></tr>
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `${data.reportType}_Discharge_Report_${new Date().toISOString().split('T')[0]}.xls`;
        link.click();
    });
    ...
    $('#exportPdf').click(async function () {
        ...
        doc.setFontSize(14);
        doc.text(`${data.reportType} Discharge Report`, pageWidth / 2, 65, { align: 'center' });
        ...
        // Save
        doc.save(`${data.reportType}_Discharge_Report_${new Date().toISOString().split('T')[0]}.pdf`);
    });
});