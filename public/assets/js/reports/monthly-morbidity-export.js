$(document).ready(function () {
    ...
    function getMorbidityTableData() {
        const dateRange = $('.date-range-display').text().trim() || 'All Dates';
        const table = document.querySelector('table.table-row-dashed');

        if (!table) {
            alert('No morbidity records found.');
            return null;
        }
    ...
    $('#exportCsv').click(function () {
        const data = getMorbidityTableData();
        if (!data) return;
        ...
        link.download = `Morbidity_Report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    });
    ...
    $('#exportExcel').click(function () {
        ...
        let tableHTML = `
            <table border="1">
                <tr><th colspan="${data.headers.length}" style="font-size:16px">${APP_NAME} - Monthly Outpatient Morbidity Returns</th></tr>
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `Morbidity_Report_${new Date().toISOString().split('T')[0]}.xls`;
        link.click();
    });
    ...
    $('#exportPdf').click(function () {
        ...
        doc.setFontSize(14);
        doc.text('Monthly Outpatient Morbidity Returns', pageWidth / 2, 60, { align: 'center' });
        ...
        doc.save(`Morbidity_Report_${new Date().toISOString().split('T')[0]}.pdf`);
    });
    ...
    $('#printReport').click(function () {
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <h3 style="text-align:center;">Monthly Outpatient Morbidity Returns</h3>
