            ...
            <div class="row mb-5">
                <div class="col-md-12 d-flex flex-wrap">
                    <div class="input-group mb-0">
                        <input type="text" class="form-control" placeholder="Search Patient or Bill Number"
                ...
                <div id="pharmacyBillPrintSection">
                    <div class="table-responsive">
                        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                            ...
                            <tbody>
                                @forelse($pharmacyBills['data'] as $bill)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="image image-circle image-mini me-3">
                                                    <a href="{{ route('patients.show', $bill['patient']['id']) }}">
                                                        <img src="{{ $bill['patient']['image_url'] }}" alt="" class="user-img object-contain image rounded-circle">
                                                    </a>
                                                </div>
                                                <div class="d-flex flex-column">
                                                    <a href="{{ route('patients.show', $bill['patient']['id']) }}" class="text-decoration-none mb-1">{{ $bill['patient']['name'] }}</a>
                                        ...
                                        <td>
                                            @if($bill['doctor']['id'])
                                                <div class="d-flex align-items-center">
                                                    <div class="image image-circle image-mini me-3">
                                                        <a href="{{ url('doctors') . '/' . $bill['doctor']['id'] }}">
                                                            <img src="{{ $bill['doctor']['image_url'] }}" alt="" class="user-img object-contain image rounded-circle">
                                                        </a>
                                                    </div>
                                                    <div class="d-flex flex-column">
                                                        <a href="{{ url('doctors') . '/' . $bill['doctor']['id'] }}" class="text-decoration-none mb-1">{{ $bill['doctor']['name'] }}</a>
                                                        <span>{{ $bill['doctor']['email'] }}</span>
                                                    </div>
                                                </div>
                                            @else
                                                N/A
                                        ...
                                        <td class="text-end">{{ getCurrencySymbol() }} {{ number_format($bill['total_amount'], 2) }}</td>
                                        <td class="text-end">{{ getCurrencySymbol() }} {{ number_format($bill['paid_amount'], 2) }}</td>
                                        <td class="text-end">{{ getCurrencySymbol() }} {{ number_format($bill['balance'], 2) }}</td>
                                        <td class="text-center">
                                            @if($bill['status'] === 'paid')
                                                <span class="badge bg-light-success">Paid</span>
                                            @elseif($bill['status'] === 'partially_paid')
                                                <span class="badge bg-light-warning">Partially Paid</span>
                                            @else
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center">No pharmacy bill records found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            <tfoot>
                                <!-- <tr class="fw-bold">
                                    ...
                                    <td class="text-end">{{ getCurrencySymbol() }} {{ number_format($pharmacyBills['total_amount'], 2) }}</td>
                                    <td class="text-end">{{ getCurrencySymbol() }} {{ number_format($pharmacyBills['total_paid'], 2) }}</td>
                                    <td class="text-end">{{ getCurrencySymbol() }} {{ number_format($pharmacyBills['total_balance'], 2) }}</td>
                                    <td></td>
                                </tr> -->
                            </tfoot>
                        </table>
                    ...
                    @if($pharmacyBills['total'] > 0)
                        <div class="d-flex justify-content-between align-items-center mt-5">
                            <div>
                                {{ $pharmacyBills['paginator']->onEachSide(1)->links() }}
                            </div>
                        </div>
                    @endif
                </div>
...
<div class="d-none" id="pharmacyBillPrintSection">
        ...
        <tbody>
            @forelse($pharmacyBills['data'] as $bill)
                <tr>
                    ...
                    <td>{{ getCurrencySymbol() }} {{ number_format($bill['total_amount'], 2) }}</td>
                    <td>{{ getCurrencySymbol() }} {{ number_format($bill['paid_amount'], 2) }}</td>
                    <td>{{ getCurrencySymbol() }} {{ number_format($bill['balance'], 2) }}</td>
                    <td>
                        @if($bill['status'] === 'paid')
                            Paid
                        @elseif($bill['status'] === 'partially_paid')
            ...
            <tr class="fw-bold">
                <td colspan="4" class="text-end">Total:</td>
                <td>{{ getCurrencySymbol() }} {{ number_format($pharmacyBills['total_amount'], 2) }}</td>
                <td>{{ getCurrencySymbol() }} {{ number_format($pharmacyBills['total_paid'], 2) }}</td>
                <td>{{ getCurrencySymbol() }} {{ number_format($pharmacyBills['total_balance'], 2) }}</td>
                <td></td>
            </tr>
        </tbody>
    </table>
