...
@section('section')
    ...
    <h5 class="mb-4">{{ __('Patient ID Auto Generation') }}</h5>

    {{ Form::open([
        'route'  => ['id-settings.upsert', 'scope' => 'patient'],
        'method' => 'post',
        'id'     => 'patientIdSettingForm'
    ]) }}
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="form-group mb-5">
                {{ Form::label('enabled', __('Auto Patient ID') . ':', ['class' => 'form-label']) }}
                <span class="required"></span>
                <select name="enabled" class="form-select" required>
                    @php $isEnabled = old('enabled', optional($patientIdSetting)->enabled ?? true); @endphp
                    <option value="1" {{ $isEnabled ? 'selected' : '' }}>{{ __('Enabled') }}</option>
                    <option value="0" {{ !$isEnabled ? 'selected' : '' }}>{{ __('Disabled') }}</option>
                </select>
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="form-group mb-5">
                {{ Form::label('prefix', __('ID Prefix') . ':', ['class' => 'form-label']) }}
                <span class="required"></span>
                {{ Form::text('prefix', old('prefix', optional($patientIdSetting)->prefix ?? 'PT'), [
                    'class' => 'form-control',
                    'required',
                    'placeholder' => 'PT',
                ]) }}
        ...
        <div class="col-md-6">
            <div class="form-group mb-5">
                {{ Form::label('digits', __('ID Digits') . ':', ['class' => 'form-label']) }}
                <span class="required"></span>
                {{ Form::number('digits', old('digits', optional($patientIdSetting)->digits ?? 5), [
                    'class' => 'form-control',
                    'required',
                    'min' => 1,
                    'max' => 12,
        ...
        <div class="col-md-6">
            <div class="form-group mb-5">
                {{ Form::label('current_counter', __('Start From (Current Counter)') . ':', ['class' => 'form-label']) }}
                <span class="required"></span>
                {{ Form::number('current_counter', old('current_counter', optional($patientIdSetting)->current_counter ?? 0), [
                    'class' => 'form-control',
                    'required',
                    'min' => 0,
                    'placeholder' => '0',
                ]) }}
                <div class="form-text">{{ __('The next generated ID will be counter + 1.') }}</div>
            </div>
        </div>
    </div>
