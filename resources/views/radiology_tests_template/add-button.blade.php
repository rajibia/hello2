...
@if ($currentRoute == 'radiology.test.template.index' || $currentRoute == 'radiology.test.show' || $currentRoute == 'livewire.message')
    @if(Auth::user()->hasRole('Lab Technician'))
        <div class="dropdown">
            ...
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <li>
                    <a href="{{ route('radiology.test.templates.create') }}" class="dropdown-item px-5">
                        {{ __('messages.radiology_test.new_radiology_test_template') }}
                    </a>
                </li>
                <li>
                    <a href="{{ route('radiology.tests.excel') }}" data-turbo="false" class="dropdown-item px-5">
...
@if ((Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Doctor') || Auth::user()->hasRole('Lab Technician')) && $pId != null)
    <a href="{{ route('radiology.test.template.create', ['ref_p_id' => $pId ]) }}"
...
@if ((Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Doctor') || Auth::user()->hasRole('Lab Technician')) && $opdId != null)
    <a href="{{ route('radiology.test.template.create', ['ref_opd_id' => $opdId ]) }}"
        class="btn btn-primary">{{ __('messages.radiology_test.new_radiology_test') }}</a>
@endif

@if ((Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Doctor')) && $ipdId != null)
    <a href="{{ route('radiology.test.template.create', ['ref_ipd_id' => $ipdId ]) }}"
