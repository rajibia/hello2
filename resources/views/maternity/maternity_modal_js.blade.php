...
(function() {
    ...
    function initializeHandlers() {
        ...
        $('#createMaternityPatientModal').on('shown.bs.modal', function() {
            // Initialize Select2 for elements with data-control="select2"
            $('#createMaternityPatientModal select[data-control="select2"]').each(function() {
                ...
                $(this).select2({
                    dropdownParent: $('#createMaternityPatientModal'),
                    width: '100%'
                });
            });
        });
        ...
        $(document).on('click', '.patient-result-item', function (e) {
            ...
            if (id) {
                ...
                // For patient vitals, we'll get them from the patient's previous maternity records
                $.ajax({
        ...
        $('#createMaternityPatientModal').on('show.bs.modal', function() {
            // Reset form fields if needed
            $('#createMaternityPatientForm').trigger('reset');
        ...
        $('#saveMaternityPatientBtn').off('click');
        ...
        $('#saveMaternityPatientBtn').on('click', function() {
            // Trigger our custom event instead of form submit
            $('#createMaternityPatientForm').trigger('submitMaternityForm');
        });

        // Handle modal close to clean up any Select2 instances
        $('#createMaternityPatientModal').on('hidden.bs.modal', function() {
            // Clean up any Select2 instances to prevent memory leaks
            $('#createMaternityPatientModal select').each(function() {
                if ($(this).hasClass('select2-hidden-accessible')) {
                    $(this).select2('destroy');
                }
            });
        ...
        $('#createMaternityPatientForm').off('submitMaternityForm');

        // Form submission - using custom event to avoid duplicate submissions
        $('#createMaternityPatientForm').on('submitMaternityForm', function (e) {
            e.preventDefault();
            ...
            $('#saveMaternityPatientBtn').prop('disabled', true);
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success) {
                        ...
                        $('#createMaternityPatientModal').modal('hide');
                ...
                error: function (result) {
                    ...
                    $('#saveMaternityPatientBtn').prop('disabled', false);
                },
                complete: function () {
                    $('#saveMaternityPatientBtn').prop('disabled', false);
                }
