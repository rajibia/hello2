            ...
            <div class="text-end mt-4 mt-md-0">
                @if(isset($medicineBill->payment_status) && $medicineBill->payment_status == false)
                    <a class="btn btn-primary edit-btn"
                        href="{{route('accounts-medicine-bills.edit', ['accounts_medicine_bill' => $medicineBill->id]) }}">{{ __('messages.common.edit') }}</a>
                @endif
                <a href="{{route('accounts-medicine-bills.index')}}"
...
@section('content')
    <div class="container-fluid">
        <div class="d-flex flex-column">
            <div class="card">
                <div class="card-body">
                    @include('accounts.medicine-bills.show_fields')
                </div>
            </div>
        </div>
    </div>
