...
<li
    ...
    <a class="nav-link  d-flex align-items-center py-3" href="{{ route('duty.roster.index') }}">
            <span class="aside-menu-icon me-3">
                <i class="fas fa-clock"></i>
            </span>
        <span class="aside-menu-title">{{ __('messages.duty-roster') }}</span>
    </a>
</li>
@endmodulePermission
{{-- Appointments --}}
...
{{-- DEBUG TAG --}}
<span style="color:red;">MENU BLADE (ORIGINAL)</span>
<li
        class="nav-item  {{ Request::is('ward-types*', 'wards*', 'ward-assigns*', 'bulk-wards', 'ward-status') ? 'active' : '' }}">
    <a class="nav-link  d-flex align-items-center py-3" href="{{ route('bed-status') }}">
        <span class="aside-menu-icon me-3"><i class="fas fa-hospital"></i></span>
        <span class="aside-menu-title">{{ __('messages.ward_management') }}</span>
        <span class="d-none">{{ __('messages.ward_types') }}</span>
        <span class="d-none">{{ __('messages.wards') }}</span>
        <span class="d-none">{{ __('messages.ward_assigns') }}</span>
    </a>
</li>
@endmodulePermission
@endif
...
@if ($insuranceMgt)
    ...
    @if (Auth::user()->email_verified_at != null)
        ...
        @if ($receptionistPathologyMgt)
            <li class="nav-item  {{ Request::is('pathology-categories*', 'pathology-tests*', 'pathology-tests-templates*') ? 'active' : '' }}">
        ...
        @if ($receptionistServiceMgt)
            <li
                class="nav-item {{ Request::is('services*', 'insurances*', 'scans*', 'labs*', 'procedures*', 'packages*') ? 'active' : '' }}">
