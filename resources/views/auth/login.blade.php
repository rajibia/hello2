...
@section('content')
    ...
    <div class="row" style="height: 100vh; background: #eff3f7;">
        <div class="col-6" style="background-image: url('{{ url('/uploads/login-bg.jpg') }}'); background-position: center; background-size: cover;">
            ...
            <div class="bg-white rounded-15 shadow-md width-540 px-5 px-sm-7 py-10 mx-auto">
                ...
                <form method="post" action="{{ url('/login') }}">
                    ...
                    <div class="mb-sm-7 mb-4">
                        <label for="email" class="form-label">{{ __('auth.email') . ':' }}<span class="required"></span></label>
                        <input name="email" type="email" class="form-control" id="email" required
                               placeholder="{{ __('auth.email') }}" value="{{ Cookie::get('email') ?? old('email') }}">
                    </div>
                    <div class="mb-sm-7 mb-4">
                        <div class="d-flex justify-content-between">
                            <label for="password" class="form-label">{{ __('auth.password') . ':' }}<span class="required"></span></label>
    ...
    <!-- Include reCAPTCHA JS at the bottom of the page -->
    {!! NoCaptcha::renderJs() !!}
@endsection
