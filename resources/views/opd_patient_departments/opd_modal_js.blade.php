...
(function() {
    ...
    function initializeHandlers() {
        ...
        $('#createOpdPatientModal').on('shown.bs.modal', function() {
            // Initialize Select2 for elements with data-control="select2"
            $('#createOpdPatientModal select[data-control="select2"]').each(function() {
                ...
                $(this).select2({
                    dropdownParent: $('#createOpdPatientModal'),
                    width: '100%'
                });
            });
        });
        ...
        $('#createOpdPatientModal').on('shown.bs.modal', function() {
            ...
            setTimeout(function() {
                if ($('#opdAppointmentDate').length) {
                    // Destroy existing flatpickr instance if it exists
                    if ($('#opdAppointmentDate')[0]._flatpickr) {
                        $('#opdAppointmentDate')[0]._flatpickr.destroy();
                    }
                    
                    // Get current date and time
                    const now = new Date();
                    
                    $('#opdAppointmentDate').flatpickr({
                        enableTime: true,
                    ...
                    $('#opdAppointmentDate').prop('readonly', false);
                }
            }, 500);
        });
                                    ...
                                    resultsBox.append(`
                                        <a href="javascript:void(0)" class="list-group-item list-group-item-action patient-result-item" 
                                           ...
                                           data-name="${patient.first_name} ${patient.last_name}">
                                            ...
                                            <small>Phone: ${patient.phone || 'N/A'} | Gender: ${gender}</small>
                                        </a>
                                    `);
                                }
                            }
        ...
        $(document).on('click', '.patient-result-item', function (e) {
            ...
            if (id) {
                $.ajax({
                    url: $('#createOpdPatientCasesUrl').val(),
                    type: 'get',
                    dataType: 'json',
                    data: { id: id },
                    success: function(data) {
            ...
            if (doctorId !== '') {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#opdChargeId').html(`<option value="1">${doctorName}</option>`).prop('disabled', true);
                        
                        // Set standard charge or default to 0
                        $('#opdStandardCharge').val(response.standard_charge || '0.00');
                    },
                    error: function() {
                        $('#opdChargeId').html('<option value="">Error loading charges</option>')
                            .prop('disabled', false);
                ...
                $('#opdChargeId').prop('disabled', true);
            }
        });
        
        // Reset form and reinitialize components when modal is opened
        $('#createOpdPatientModal').on('show.bs.modal', function() {
            // Reset form fields if needed
            $('#createOpdPatientForm').trigger('reset');
        ...
        $('#saveOpdPatientBtn').on('click', function() {
            // Trigger our custom event instead of form submit
            $('#createOpdPatientForm').trigger('submitOpdForm');
        });
        
        // Handle modal close to clean up any Select2 instances
        $('#createOpdPatientModal').on('hidden.bs.modal', function() {
            // Clean up any Select2 instances to prevent memory leaks
            $('#createOpdPatientModal select').each(function() {
                if ($(this).hasClass('select2-hidden-accessible')) {
                    $(this).select2('destroy');
                }
            });
        ...
        $('#createOpdPatientForm').off('submitOpdForm');
        
        // Form submission - using custom event to avoid duplicate submissions
        $('#createOpdPatientForm').on('submitOpdForm', function (e) {
            e.preventDefault();
            ...
            $('#saveOpdPatientBtn').prop('disabled', true);
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success) {
                        ...
                        $('#createOpdPatientModal').modal('hide');
                        
                        // Emit Livewire event to refresh the table instead of full page reload
                        if (typeof window.Livewire !== 'undefined') {
                            // Get the current filter from URL if it exists
                            const urlParams = new URLSearchParams(window.location.search);
                            const currentFilter = urlParams.get('filter') || 'upcoming';
                            
                            // Emit refresh event to the Livewire component
                            window.Livewire.emit('refresh');
                            
                            // If we're not already on the upcoming filter page, redirect there
                            if (!window.location.href.includes('filter=upcoming')) {
                                window.location.href = '/opds?filter=upcoming';
                            }
                        } else {
                            // Fallback to traditional page reload if Livewire is not available
                            window.location.href = '/opds?filter=upcoming';
                        }
                    }
                },
                error: function (result) {
                    ...
                    $('#saveOpdPatientBtn').prop('disabled', false);
                },
                complete: function () {
                    $('#saveOpdPatientBtn').prop('disabled', false);
                }
