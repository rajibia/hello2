            ...
            <tbody>
                @foreach($users as $user)
                    @php
                        ...
                        $maternityRecord = $maternityPatientDepartments->firstWhere('patient_id', $patientId);
                        $maternityPatientCase = $maternityPatientCases->firstWhere('patient_id', $patientId);
                    @endphp
                    <tr>
                        <td>
                            @if($patientId)
                                <input type="radio" name="selected_patient" 
                                    ...
                                    onchange="selectPatient(
                                        ...
                                        '{{ $maternityRecord?->maternity_number ?? '' }}',
                                        '{{ $maternityPatientCase?->case_id ?? '' }}',
                                        '{{ $maternityRecord?->height ?? 0 }}',
                                        '{{ $maternityRecord?->weight ?? 0 }}',
                                        '{{ $maternityRecord?->bp ?? '' }}',
                                        '{{ $maternityRecord?->temperature ?? '' }}',
                                        '{{ $maternityRecord?->respiration ?? '' }}'
                                    )">
                            @endif
                        </td>
                        ...
                        <td>{{ $maternityPatientCase?->case_id ?? 'N/A' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
...
<script>
function selectPatient(patientId, patientName, maternityNumber, caseId, height, weight, bp, temperature, respiration) {
