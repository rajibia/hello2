...
function loadSuperAdminDashboardData () {
    ...
    console.log('Chart canvas found:', $('#incomeExpenseChart').length);

    if(!$('#incomeExpenseChart').length){
        console.log('Chart canvas not found, returning');
        return
    }

    incomeChartCanvasAttr = $('#incomeExpenseChart')
    console.log('Starting dashboard chart...');
...
const dashboardChart = () => {
    console.log('Making AJAX call to dashboard chart...');
    $.ajax({
        type: 'post',
...
const dashboardPlanIncomeChart = (data) => {
    ...
    if (!data || !data.days || !data.income || !data.expense) {
