    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                $("#editOpdDiagnosisReportBp").val(result.data.bp);
                $("#editOpdDiagnosisReportPulse").val(result.data.pulse);
                $("#editOpdDiagnosisReportRespiration").val(result.data.respiration);
                $("#editOpdDiagnosisReportTemperature").val(result.data.temperature);
                $("#editOpdDiagnosisReportOxygenSaturation").val(result.data.oxygen_saturation);
                document
                    .querySelector("#editOpdDiagnosisReportDate")
                    ._flatpickr.setDate(
                        moment(result.data.report_date).format()
                    );
                $("#editOpdDiagnosisName").val(result.data.name);
                $("#editOpdDiagnosisCode").val(result.data.code);
...
listenClick(".viewOpdDiagnosisBtn", function (event) {
    if ($(".ajaxCallIsRunning").val()) {
        return;
    }
    ajaxCallInProgress();
    let opdDiagnosisId = $(event.currentTarget).attr("data-id");
    renderOpdDiagnosisDataView(opdDiagnosisId);
});

window.renderOpdDiagnosisDataView = function (id) {
    $.ajax({
        url: $("#showOpdDiagnosisUrl").val() + "/" + id + "/edit",
        type: "GET",
        success: function (result) {
            if (result.success) {
                ...
                if (result.data.report_type != "") {
                    $("#opdDiagnosisReportTypeView").text(result.data.report_type);
                } else {
                    $("#opdDiagnosisReportTypeView").text('N/A');
                }
                if (result.data.report_date !== "" && result.data.report_date !== null) {
                    ...
                    $("#opdDiagnosisReportDateView").html(`
                    ...
                    $("#opdDiagnosisReportDateView").text('N/A');
                }
                if (result.data.description != "" && result.data.description != null) {
                    $("#opdDiagnosisDescriptionView").text(result.data.description);
                } else {
                    $("#opdDiagnosisDescriptionView").text('N/A');
                }
               
                if (result.data.ipd_diagnosis_document_url != "" && result.data.ipd_diagnosis_document_url != null) {
                    $("#opdDiagnosisDocumentView").attr(
                        "href",
                        result.data.ipd_diagnosis_document_url
                    );
                } 
                $("#show_opd_diagnosis_modal").modal("show");
                ajaxCallCompleted();
            }
        },
        error: function (result) {
