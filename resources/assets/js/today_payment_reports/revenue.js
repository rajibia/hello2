...
document.addEventListener("turbo:load", loadRevenueTable);

function loadRevenueTable() {
    let revenueTimeRange = $("#time_range-revenue");
    var now = moment();

// Set the start of the revenue to the current moment
var revenueStart = now;

// Set the end of the revenue to tomorrow at the same time
var revenueEnd = now.clone().add(1, 'day');
    let revenueStartTime = "";
    let revenueEndTime = "";

 
    function cb(revenueStart, revenueEnd) {
        $("#time_range-revenue").val(revenueStart.format("MM/DD/YYYY") + " - " + revenueEnd.format("MM/DD/YYYY"));
    }

    if (revenueTimeRange.length) {
        Lang.setLocale($(".userCurrentLanguage").val());
        revenueTimeRange.daterangepicker(
            {
        ...
        cb(revenueStart, revenueEnd);
        revenueTimeRange.on("apply.daterangepicker", function (ev, picker) {
            revenueStartTime =
                picker.startDate.format("YYYY-MM-D  H:mm:ss");
            revenueEndTime = picker.endDate.format("YYYY-MM-D  H:mm:ss");
    ...
    listenClick("#revenueResetFilter", function () {
        let revenueTimeRange = $("#time_range-revenue");
        revenueStartTime = revenueTimeRange
            .data("daterangepicker")
            .setStartDate(moment().startOf("week").format("MM/DD/YYYY"));
        revenueEndTime = revenueTimeRange
            .data("daterangepicker")
            .setEndDate(moment().endOf("week").format("MM/DD/YYYY"));
        hideDropdownManually($("#revenueFilterBtn"), $(".dropdown-menu"));
    });
