...
class IpdPatientDepartment extends Model
{
    ...
    public static $rules = [
        ...
        'discharge_notes' => 'nullable',
        'discharge_date' => 'nullable',
        'doctor_discharge' => 'nullable',
    ...
    protected $casts = [
        ...
        'doctor_discharge' => 'integer',
        'doctor_incharge' => 'integer'
    ];
    ...
    {
        return $this->belongsTo(User::class, 'doctor_incharge');
    }
    
    // Custom accessor for discharge status display
    public function getDischargeStatusDisplayAttribute()
    {
        if ($this->bill_status == 1) {
            return '<span class="text-success">Yes</span> (<small>' . ($this->discharge_status ?? '') . '</small>)';
        } elseif ($this->doctor_discharge == 1) {
            return '<span class="text-success">Partial</span>';
        } else {
