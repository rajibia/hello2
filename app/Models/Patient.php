...
class Patient extends Model
{
    ...
    // {
    //     $patientAdmissionId = $this->idGenerator->next() . Str::random(6);
    //     while (true) {
    ...
    public static function generateUniquePatientId()
    {
        ...
        // $idGenerator = app(PatientIdGenerator::class);

        // Generate the next base ID + random suffix
        $patientAdmissionId =  Str::random(6);

        // Ensure uniqueness
        while (self::where('patient_unique_id', $patientAdmissionId)->exists()) {
            $patientAdmissionId = Str::random(6);
        }

        return $patientAdmissionId;
    }
    public function medicine_bills(): HasMany
    {
        return $this->hasMany(MedicineBill::class, 'patient_id');
    }
    public function ipd_bills(): HasMany
    {
        return $this->hasMany(IpdPatientDepartment::class, 'patient_id');
    }
    ...
    public function ipdPatientDepartments()
    {
        return $this->hasMany(IpdPatientDepartment::class, 'patient_id');
    }
    ...
    {
        return $this->hasMany(Maternity::class, 'patient_id');
    }
    ...
    public function maternityDepartment(): HasOne
    {
        return $this->hasOne(Maternity::class, 'patient_id');
    }
    ...
    {
        $invoice_amount = Invoice::where('patient_id', $this->id)->where('status', 1)->sum('amount');
        $invoice_discount = Invoice::where('patient_id', $this->id)->where('status', 1)->sum('discount');

        $bills_amount = Bill::whereNull('status')->sum('amount');

        $total_sum = $invoice_amount - $invoice_discount + $bills_amount;
        return $total_sum;
    }

    /**
     * Get the antenatal records for the patient.
     */
    ...
    public function opdPostnatals()
    {
        return $this->hasMany(PostnatalHistory::class);
    }

    public function ipdPostnatals()
    {
        return $this->hasMany(IpdPostnatalHistory::class);
    }

    public function ipdPreviousObstetricHistory()
    {
        return $this->hasMany(PreviousObstetricHistory::class);
    }
