...
{
    $totalSum = 0;
    $amount = Invoice::where('invoices.status', 0)->whereDate('invoice_date', Carbon::today())->get();
...
{
    $patientCase = PatientCase::with('patient.patientUser')
        ->where('doctor_id', '=', $userOwnerId)
        ->where('status', '=', 1)
        ->get()
        ->pluck('patient.user_id', 'id');

    $patientAdmission = PatientAdmission::with('patient.patientUser')
        ->where('doctor_id', '=', $userOwnerId)
        ->where('status', '=', 1)
        ->get()
        ->pluck('patient.user_id', 'id');
    ...
    $patients = Patient::with('patientUser')
        ...
        ->sortBy(function ($patient) {
            // Sort by placing patients with full_name 'Walk In' first
            return $patient->patientUser->full_name === 'Walk In' ? 0 : 1;
        })
...
function getMenuLinks($menu)
{
    ...
    if ($menu == User::MAIN_MATERNITY) {
        $defaultRoute = route('maternity.index');
        $subMenus = ['Maternity'];
    }
    ...
    if ($menu == User::MAIN_PATHOLOGY) {
        $defaultRoute = route('pathology.test.index');
        $subMenus = ['Pathology Categories', 'Pathology Tests', 'Pathology Units', 'Pathology Parameters', 'Dynamic Pathology Templates'];
    }
    // Report
    ...
    if ($menu == User::MAIN_SERVICE) {
        ...
        $subMenus = ['Insurance Setup', 'OPD Services', 'Scans', 'Laboratory', 'Procedure', 'Packages'];//

    }

    // Ambulance
    if ($menu == User::MAIN_AMBULANCE) {
        $defaultRoute = route('ambulances.index');
        $subMenus = ['Ambulances', 'Ambulance Calls'];//
    }

    // Insuarance
    if ($menu == User::MAIN_INSURANCE) {
        $defaultRoute = route('insurances.index');
        $subMenus = ['Insurances'];
        // $subMenus = ['Insurance Setup', 'OPD Services', 'Scans', 'Laboratory', 'Procedure', 'Packages'];
    }
...
function getPaymentCredentials($key)
{
    ...
    if (!empty($query)) {
        // Check if the key exists in the $query array before accessing it
