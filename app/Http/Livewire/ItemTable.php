...
class ItemTable extends LivewireTableComponent
{
    ...
    public function configure(): void
    {
        ...
        $this->setTdAttributes(function (Column $column, $row, $columnIndex, $rowIndex) {
            if ($column->isField('name') || $column->isField('unit.name') || $column->isField('available_quantity')) {
                return [
                    'class' => 'p-5',
                ];
            }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.item.item_category'), 'itemcategory.name')
                ->searchable()
                ->sortable(),
            Column::make(__('messages.item.unit'), 'unit.name')
    ...
    {
        /** @var Item $query */
        return Item::with(['itemcategory', 'unit'])->select('items.*');
    }
}
