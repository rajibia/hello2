...
class ServiceTable extends LivewireTableComponent
{
    ...
    public function configure(): void
    {
        ...
        $this->setTdAttributes(function (Column $column, $row, $columnIndex, $rowIndex) {
            ...
            if ($column->isField('charge_status') || $column->isField('insurance_name') || $column->isField('age') || $column->isField('non_insured_amount') || $column->isField('name') || $column->isField('rate') || $column->isField('status')) {
                return [
                    'class' => 'p-3',
                ];
            }
    ...
    public function columns(): array
    {
        return [
            Column::make(__('messages.package.service'), 'name')
                ->view('services.templates.columns.name')
                ->sortable()->searchable(),
            // Column::make(__('messages.service.quantity'), 'quantity')->view('services.templates.columns.quantity')
            //     ->sortable()->searchable(),
            Column::make('Insurance tarriff', 'rate')->view('services.templates.columns.rate')
                ->sortable()->searchable(),
            Column::make('Non-insured amount', 'non_insured_amount')->view('services.templates.columns.non_insured_amount')
                ->sortable()->searchable(),
            Column::make('Age', 'age')
                ->sortable()->searchable(),
            Column::make('Charge status', 'charge_status')
                ->sortable()->searchable(),
