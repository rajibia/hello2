...
class PatientTable extends LivewireTableComponent
{
    ...
    {
        // $idSetting = \App\Models\IdSetting::where('scope', 'patient')->first();

        // if ($idSetting) {
        //     $this->idPrefix = $idSetting->prefix . str_pad(
        //         $idSetting->current_counter,
        //         $idSetting->digits,
        //         '0',
        //         STR_PAD_LEFT
        //     );
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.user.phone'), 'patientUser.phone')->view('patients.columns.phone')
                ->sortable()->searchable(),
            Column::make(__('messages.user.gender'), 'patientUser.gender')->view('patients.columns.gender')
                ->sortable()->searchable(),
            Column::make(__('messages.amount_due'), 'patient_unique_id')->view('patients.columns.amount_due')
                ->sortable()->searchable(),
            Column::make(__('messages.user.blood_group'), 'patientUser.blood_group')->view('patients.columns.blood_group')
                ->sortable()->searchable(),
            Column::make('Category', 'company_id')->view('patients.columns.category')
                ->sortable(),
                // ->searchable(),
