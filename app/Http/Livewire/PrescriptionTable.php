...
class PrescriptionTable extends LivewireTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.prescription.medical_history'), 'created_at')
            ...
            Column::make(__('messages.issue_status'), 'medicineBill.payment_status')
                ->view('prescriptions.templates.columns.payment_status'),
    ...
    public function builder(): Builder
    {
        ...
        if (! getLoggedinDoctor()) {
            $query = Prescription::query()->select('prescriptions.*')->with('patient', 'doctor', 'medicineBill');
        } else {
            $doctorId = Doctor::where('user_id', getLoggedInUserId())->first();
            $query = Prescription::query()->select('prescriptions.*')->with('patient', 'doctor', 'medicineBill')->where('prescriptions.doctor_id',
                $doctorId->id);
        }
