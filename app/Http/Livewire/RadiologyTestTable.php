...
class RadiologyTestTable extends Component
{
    ...
    {
        $this->available_templates = RadiologyTestTemplate::where('is_dynamic_form', true)
    ...
    {
        if (!empty($value)) {
            $template = RadiologyTestTemplate::find($value);
            if ($template) {
    ...
    public function store()
    {
        try {
            ...
            foreach ($this->selected_tests as $testId => $testData) {
                ...
                $template = RadiologyTestTemplate::find($testData['template_id']);
                if (!$template) {
                    continue;
                }
            ...
            // Create a single radiology test record
            ...
            // Create radiology test items for each test
            $createdItems = [];
            foreach ($validTests as $item) {
                $template = $item['template'];

                $radiologyTestItem = \App\Models\RadiologyTestItem::create([
                ...
                $createdItems[] = $radiologyTestItem;
            }

            Log::info('Radiology test request created successfully', [
            ...
            session()->flash('message', 'Radiology test request created successfully with ' . count($createdItems) . ' test(s). Total: GHS ' . number_format($grandTotal, 2) . ' (after ' . $discountPercent . '% discount)');
            $this->showCreateModal = false;
            $this->resetForm();
            $this->refreshTests();

        } catch (\Exception $e) {
            Log::error('Failed to create radiology test request: ' . $e->getMessage());
            session()->flash('error', 'Failed to create radiology test request: ' . $e->getMessage());
        } finally {
    ...
    {
        $lastTest = RadiologyTest::latest()->first();
        $lastNumber = $lastTest ? intval(substr($lastTest->bill_no, 3)) : 0;
        $newNumber = $lastNumber + 1;

        return 'RT' . str_pad($newNumber, 6, '0', STR_PAD_LEFT);
    }

    public function edit($id)
    {
        try {
            $this->editingTest = RadiologyTest::with(['radiologyTestItems.radiologytesttemplate', 'patient', 'doctor'])->findOrFail($id);
            ...
            if ($this->editingTest->radiologyTestItems && $this->editingTest->radiologyTestItems->count() > 0) {
                foreach ($this->editingTest->radiologyTestItems as $testItem) {
            ...
            Log::info('Radiology test loaded for editing', [
                'test_id' => $id,
                'patient_id' => $this->patient_id,
                'doctor_id' => $this->doctor_id,
                'case_id' => $this->case_id,
            ...
            Log::error('Failed to load radiology test for editing: ' . $e->getMessage());
            session()->flash('error', 'Failed to load radiology test for editing: ' . $e->getMessage());
        }
    }
    ...
    public function update()
    {
        try {
            ...
            // Update or create radiology test items
            $existingItemIds = [];
            foreach ($this->edit_selected_tests as $testData) {
                if (!empty($testData['template_id'])) {
                    if (isset($testData['item_id'])) {
                        // Update existing item
                        \App\Models\RadiologyTestItem::where('id', $testData['item_id'])->update([
                        ...
                        // Create new item
                        \App\Models\RadiologyTestItem::create([
                            'radiology_id' => $this->editingTest->id,
                            'test_name' => $testData['template_id'],
                            'report_date' => $testData['report_date'] ?? $this->expected_date,
                        ]);
                    }
            ...
            \App\Models\RadiologyTestItem::where('radiology_id', $this->editingTest->id)
                ->whereNotIn('id', $existingItemIds)
                ->delete();

            Log::info('Radiology test updated successfully', [
            ...
            session()->flash('message', 'Radiology test updated successfully with ' . count($this->edit_selected_tests) . ' test(s). Total: GHS ' . number_format($grandTotal, 2));
            $this->closeEditModal();
            $this->refreshTests();

        } catch (\Illuminate\Validation\ValidationException $e) {
            Log::error('Validation failed for radiology test update: ' . json_encode($e->errors()));
            session()->flash('error', 'Please check the form and try again.');
        } catch (\Exception $e) {
            Log::error('Failed to update radiology test: ' . $e->getMessage());
            session()->flash('error', 'Failed to update radiology test: ' . $e->getMessage());
        } finally {
            $this->isSubmitting = false;
        }
    }
    ...
    public function delete($id)
    {
        try {
            ...
            session()->flash('message', 'Radiology test request deleted successfully.');
            $this->refreshTests();
        } catch (\Exception $e) {
            Log::error('Failed to delete radiology test request: ' . $e->getMessage());
            session()->flash('error', 'Failed to delete radiology test request.');
        }
    }
    ...
    public function viewResults($id)
    {
        try {
            ...
            $this->selectedTest = RadiologyTest::with(['radiologyTestItems.radiologytesttemplate', 'patient.patientUser', 'doctor.doctorUser', 'performed_by_user'])
                ->findOrFail($id);

            Log::info('Test loaded successfully: ' . $this->selectedTest->bill_no);
            ...
            Log::info('Is Company Patient: ' . ($isCompanyPatient ? 'YES' : 'NO'));

            if ($balance > 0 && !$isCompanyPatient) {
                Log::info('Bill not paid, showing error');
            ...
            if ($this->selectedTest->performed_by_user) {
                $this->performed_by = $this->selectedTest->performed_by_user->full_name ?? $this->selectedTest->performed_by_user->name ?? '';
            } else {
                $this->performed_by = auth()->user()->full_name ?? auth()->user()->name ?? '';
            }

            // Ensure each test item has a results section
            if ($this->selectedTest->radiologyTestItems && $this->selectedTest->radiologyTestItems->count() > 0) {
                foreach ($this->selectedTest->radiologyTestItems as $testItem) {
            ...
            Log::info('Setting showResultsModal to true');
            $this->showResultsModal = true;
            Log::info('Modal should be visible now');

        } catch (\Exception $e) {
            Log::error('Failed to load radiology test results: ' . $e->getMessage());
            Log::error('Stack trace: ' . $e->getTraceAsString());
            session()->flash('error', 'Failed to load radiology test results.');
        }
    }
    ...
    public function getSpeciesOptions($testItemId, $selectedResult)
    {
        ...
        $testItem = $this->selectedTest->radiologyTestItems->where('id', $testItemId)->first();
        if (!$testItem) {
            return ['N/A'];
        }

        $formConfig = $testItem->radiologytesttemplate->form_configuration ?? [];
    ...
    {
        $options = [];
        if ($this->selectedTest && $this->selectedTest->radiologyTestItems) {
            foreach ($this->selectedTest->radiologyTestItems as $testItem) {
                $formConfig = $testItem->radiologytesttemplate->form_configuration ?? [];
    ...
    {
        try {
            $radiologyTest = RadiologyTest::findOrFail($testId);
            ...
            session()->flash('message', 'Radiology test request accepted successfully. Status changed to In Progress.');
    ...
    {
        // Debug: Log the ipdId value
        Log::info('RadiologyTestTable render - ipdId: ' . ($this->ipdId ?? 'null'));

        $query = RadiologyTest::with(['radiologyTestItems.radiologytesttemplate', 'patient.patientUser', 'doctor.doctorUser'])
        ...
        if ($this->search) {
            $query->where('bill_no', 'like', '%' . $this->search . '%')
                ->orWhereHas('patient.patientUser', function ($q) {
                    $q->where('full_name', 'like', '%' . $this->search . '%');
                })
                ->orWhereHas('radiologyTestItems.radiologytesttemplate', function ($q) {
        ...
        // Load case IDs for the dropdown
        $caseIds = \App\Models\PatientCase::pluck('case_id', 'id');

        // Load OPD and IPD data if needed
        $opds = collect();
        $ipds = collect();
        ...
        if ($this->patient_id) {
            ...
            if ($patient) {
                $opds = $patient->opd()->pluck('opd_number', 'id');
                $ipds = $patient->ipdPatientDepartments()->pluck('ipd_number', 'id');
            }
        }
