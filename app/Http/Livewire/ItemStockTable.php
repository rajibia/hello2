...
class ItemStockTable extends LivewireTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.item_stock.item_category'), 'item.itemcategory.name')
                ->searchable()
                ->sortable(),
            Column::make(__('messages.item_stock.supplier_name'), 'supplier.name')
                ->searchable()
                ->sortable(),
            Column::make(__('messages.item_stock.store_name'), 'store.name')
                ->searchable()
                ->sortable(),
    ...
    {
        /** @var ItemStock $query */
        return ItemStock::with(['item', 'supplier', 'store'])->select('item_stocks.*');
    }
}
