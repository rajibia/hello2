...
class DepartmentController extends AppBaseController
{
    ...
    {
        // If all_users parameter is passed, return all active users
        if ($request->get('all_users')) {
            $usersData = User::where('status', 1)->pluck('full_name', 'id');
            return $this->sendResponse($usersData, 'Retrieved successfully');
        }

        if (empty($request->get('id'))) {
            return $this->sendError(__('messages.users').' '.__('messages.common.not_found'));
        }
