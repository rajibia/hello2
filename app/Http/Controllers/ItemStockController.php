...
class ItemStockController extends AppBaseController
{
    ...
    public function create()
    {
        ...
        $suppliers = \App\Models\Supplier::where('status', \App\Models\Supplier::ACTIVE)
            ->orderBy('name')
            ->pluck('name', 'id')
            ->toArray();
            
        // Get all active stores for dropdown
        $stores = Store::where('status', Store::ACTIVE)
            ->orderBy('name')
            ->pluck('name', 'id')
            ->toArray();

        return view('item_stocks.create', compact('itemCategories', 'suppliers', 'stores'));
    }
    ...
    public function edit(ItemStock $itemStock)
    {
        ...
        return view('item_stocks.edit', compact('itemCategories', 'itemStock', 'suppliers', 'stores'));
    }
