...
class MedicineBillRepository extends BaseRepository
{
    ...
    public function update($medicineBill, $input)
    {
        ...
        try {
            ...
            $input['issue_status'] = isset($input['issue_status']) ? 1 : $medicineBill->issue_status;
            ...
            foreach ($input['medicine'] as $key => $value) {
                ...
                if ($input['issue_status'] == true && $medicine->available_quantity < $qty && $medicineBill->issue_status == 0) {
                    $available = $medicine->available_quantity == null ? 0 : $medicine->available_quantity;
                    throw new UnprocessableEntityHttpException(__('messages.medicine_bills.available_quantity').' '.$medicine->name.' '.__('messages.medicine_bills.is').' '.$available.'.');
                }

                if (! is_null($saleMedicine) && $input['issue_status'] == 1 && $medicineBill['issue_status'] == 1) {
                ...
                if (! array_key_exists($input['medicine'][$key], $result) && $medicine->available_quantity < $qty && $input['issue_status'] == false) {
                    $available = $medicine->available_quantity == null ? 0 : $medicine->available_quantity;
                    throw new UnprocessableEntityHttpException(__('messages.medicine_bills.available_quantity').' '.$medicine->name.' '.__('messages.medicine_bills.is').' '.$available.'.');
                }
            ...
            // Validate issue status based on patient type and payment status
            $patient = \App\Models\Patient::with('company')->find($input['patient_id']);
            if (!$patient) {
                throw new UnprocessableEntityHttpException('Patient not found.');
            }

            // Check if individual patient is trying to issue without payment
            if ((!$patient->company_id || $patient->company_id == null) &&
                $input['issue_status'] == 1 &&
                (!$input['payment_status'] || $input['payment_status'] == 0)) {
                throw new UnprocessableEntityHttpException('Individual patients cannot issue medicines without payment. Please mark payment as completed first.');
            }

            $beforeStatus = $medicineBill['payment_status'];
            $beforeIssueStatus = $medicineBill['issue_status'];
            $medicineBill->Update([
                'patient_id' => $input['patient_id'],
                'net_amount' => $input['net_amount'],
                'discount' => $input['discount'],
                'payment_status' => $input['payment_status'],
                'issue_status' => $input['issue_status'],
                'payment_type' => $input['payment_type'],
                'total' => $input['total'],
                'tax_amount' => $input['tax'],
                'note' => $input['note'],
            ...
            if ($input['category_id']) {
                foreach ($input['category_id'] as $key => $value) {
                    ...
                    if ($input['issue_status'] == 1 && $beforeIssueStatus == 0
                    ) {
                        $medicine->update([
                            'available_quantity' => $medicine->available_quantity - $input['quantity'][$key],
                        ]);
