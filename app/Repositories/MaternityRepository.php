...
use App\Models\MaternityTimeline;
use Exception;
...
class MaternityRepository extends BaseRepository
{
     ...
     public function getAssociatedData($selectedPatientId = null)
    {
        ...
        $data['maternityNumber'] = Maternity::generateUniqueMaternityNumber();
        $data['paymentMode'] = Maternity::PAYMENT_MODES;
    ...
    public function getMaternityTimeline($id)
    {
        if (\Auth::user()->hasRole('Admin')) {
            return MaternityTimeline::where('maternity_id', $id)->latest()->take(2)->get();
        }

        return MaternityTimeline::where('maternity_id', $id)->latest()->take(2)->visible()->get();
    }
    ...
    public function storeWithInvoice($input)
    {
        try {
            ...
            // Set default charge_id if not provided
            if (!isset($input['charge_id']) || empty($input['charge_id'])) {
                $input['charge_id'] = 1; // Default charge ID
            }

            // Set standard_charge from doctor's maternity charge if not provided
            if (!isset($input['standard_charge']) || empty($input['standard_charge'])) {
                if (isset($input['doctor_id']) && !empty($input['doctor_id'])) {
                    $doctorCharge = \App\Models\DoctorMaternityCharge::where('doctor_id', $input['doctor_id'])->first();
                    $input['standard_charge'] = $doctorCharge ? $doctorCharge->standard_charge : 0;
                } else {
                    $input['standard_charge'] = 0; // Default charge amount
                }
            }

            $maternity = Maternity::create($input);
            // Retrieve the ID of the created record
            $createdRecordId = $maternity->id;

            $input['patient_id'] = $maternity->patient_id;
    ...
    public function updateMaternity($input, $maternity)
    {
        try {
            $input['is_old_patient'] = isset($input['is_old_patient']) ? true : false;
            $maternity->update($input);
        } catch (Exception $e) {
            throw new UnprocessableEntityHttpException($e->getMessage());
        }
    ...
    public function createNotification($input)
    {
        try {
            ...
            if (isset($input['revisit'])) {
                ...
                $title = ($patient->patientUser->full_name ?? '').' your Maternity record has been created.';
            }
