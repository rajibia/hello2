...
class PatientCaseRepository extends BaseRepository
{
    ...
    public function store($caseInput)
    {
        try {
            $caseInput['case_id'] = mb_strtoupper(PatientCase::generateUniqueCaseId());
            $patientCase = PatientCase::create($caseInput);
    ...
    public function createNotification($caseInput)
    {
        try {
            $patient = Patient::with('patientUser')->where('id', $caseInput['patient_id'])->first();
            $doctor = Doctor::with('doctorUser')->where('id', $caseInput['doctor_id'])->first();
