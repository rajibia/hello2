...
class IpdPatientDepartmentRepository extends BaseRepository
{
    ...
    public function getAssociatedData()
    {
        ...
        ->mapWithKeys(function ($patient) {
            ...
            return [
                ...
                // ],
                $patient->id => $patient->patientUser->full_name . " (" . ($patient->patientUser->gender == 0 ? "Male" : "Female") . ")",
            ];
        });

        $data['patients2'] = Patient::with('patientUser')
        ->whereHas('patientUser', function ($query) {
            $query->where('status', 1);
        })
        ->get()
    ...
    public function store($input)
    {
        ...
        // $input['is_antenatal'] = isset($input['is_antenatal']) ? 1 : 0;
        $input['discharge'] = false;
        $ipdPatientDepartment = IpdPatientDepartment::create($input);

        if($ipdPatientDepartment->height > 0 || $ipdPatientDepartment->weight > 0 || $ipdPatientDepartment->bp != '' || $ipdPatientDepartment->temperature != '' || $ipdPatientDepartment->pulse != '' || $ipdPatientDepartment->respiration != '' || $ipdPatientDepartment->oxygen_saturation != '') {
            $vitals = new Vital();
            $vitals->patient_id = $ipdPatientDepartment->patient_id;
            $vitals->ipd_id = $ipdPatientDepartment->id;
            $vitals->height = $ipdPatientDepartment->height;
            $vitals->weight = $ipdPatientDepartment->weight;
            $vitals->bp = $ipdPatientDepartment->bp;
            $vitals->temperature = $ipdPatientDepartment->temperature;
            $vitals->pulse = $ipdPatientDepartment->pulse;
            $vitals->respiration = $ipdPatientDepartment->respiration;
            $vitals->oxygen_saturation = $ipdPatientDepartment->oxygen_saturation;
            $vitals->save();
        }
    ...
    public function getProvisionalIPDDiagnosis($id)
    {
        return IpdProvisionalDiagnosis::whereIpdPatientDepartmentId($id)->latest()->take(5)->get();
    }
