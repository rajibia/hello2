...
class BillRepository extends BaseRepository
{
    ...
    {
        $patients = Patient::whereHas('patientUser', function($query) {
            $query->where('status', 1);
        })->with('patientUser')->get()->pluck('patientUser.full_name', 'id')->sort();
    ...
    public function patientAdmissionDetails($inputs)
    {
        ...
        if (isset($patientAdmission->package_id)) {
            $package = Package::with('packageServicesItems.service')->find($patientAdmission->package_id);
