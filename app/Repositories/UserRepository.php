...
class UserRepository extends BaseRepository
{
    ...
    {
        try {
            // Prepare payload for normal columns only
            $payload = Arr::except($input, ['image', 'avatar_remove']);

            // Hash password if provided and not empty
            if (!empty($payload['password'])) {
                $payload['password'] = Hash::make($payload['password']);
            } else {
                unset($payload['password']); // don't null out existing password
            }

            // Update the user row
            $user = $this->update($payload, $userId); // assuming BaseRepository::update
